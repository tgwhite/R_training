...
get_monthly_returns = function(ticker, from_date = as.Date('2006-01-01'), source = 'yahoo') {
...
The next step is to get data for the S&P 500 and the risk free interest rate, which we will call the 13 week treasury bill rate. The same `get_monthly_returns` function can handle this. 

```{r}
comparison_returns_stacked = lapply(comparison_vec, get_monthly_returns) %>%
  bind_rows()
```

Running the function above resulted in a warning message that there are `NA` values in the daily data for the 13 week treasury bill rate obtained via Yahoo Finance. `NA` means "missing" in R and there are multiple ways to deal with this. As the warning says, one can exclude these values with `na.omit()` or fill in missing values via interpolation with `na.approx()` (model the series and predict within sample to fill in those values) or via filling in values with `na.fill()` which carries the last value of the series forward. 

Another approach is to use a different source for this time series -- there shouldn't be missing values. `quantmod` has other options for public data sources. The St. Louis Federal Reserve has a data repository called [FRED](https://fred.stlouisfed.org/) and `quantmod` can pull data from that repository programmatically. I went to FRED and typed in "treasury bill rate" in the seach bar and was presented with a set of options for time series that match that description. The first series is what I want: "3-Month Treasury Bill: Secondary Market Rate." Clicking the "daily" option leads me to the following view: 

![fred_screencap](FRED Treasury Bill Screencap.png)
Note the url: https://fred.stlouisfed.org/series/DTB3. The end of the URL (DTB3) represents the identifier for the time series, which is what we will pass to quantmod to get data from FRED. As an aside, this is essentially how I got data from Yahoo finance for the S&P 500 and the 3 month treasury bill rate as well. I searched for these series and found the ticker representation for those values.
...
# get the monthly risk free rate from fred. How did I know to pass "FRED"?
# ?getSymbols will tell you the options for that function, I simply
# fed it the fred option that it expected
rf_rate = get_monthly_returns('DTB3', source = 'FRED')


```
Interestingly enough, FRED also reports missing values for the treasury bill rate. Since FRED is an official repository for public data, I will trust this result a bit more, but a more comprehensive analysis would investigate exactly which values are missing and how often that happens. In this case, I will include this data in the analysis. The effect of `NA` values in modeling is that the full observation (row) associated with missing data will be excluded from analysis. There are other approaches to fill in data for missing values as alluded to above, but that's beyond the scope of this tutorial.
...
joined_comparators = 
  select(rf_rate, -ticker) %>% # get rid of the ticker column with the minus sign
  rename(
...
ggplot(all_joined_analysis_data, aes(sp500_net_rf, return_net_rf)) + 
  ...
  scale_y_continuous(labels = percent) + 
  scale_x_continuous(labels = percent) + 
  # this zooms the plot in -- there were some values way out there!
...
```
Now you have your betas saved in a list and you can do what you want with them! Do these values make sense given what was seen in the scatterplot before? These betas are all very close, but they look consistent with the tight relationships we saw in the second scatterplot showing returns net of the risk free rate. All of these stocks have a strong relationship with the market. Now you can use these betas to build a portfolio. 
